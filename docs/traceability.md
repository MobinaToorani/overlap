...
Last verified against the repo: **2026-09-04** (94 tests passing; test names below are real and were read out of the suite, not transcribed from a document).
...
From the adversarial review of the product design. *(That review's own report, `audit-report.md`, is referenced by both specs and is not yet in the repo — see Gaps.)*
...
| **A8 weekly ask, monthly behaviour** | ~~FIX-7 stepdown~~ | ⛔ | — | ⛔ **struck; A8 is REOPENED as unresolved** (ADR-0007, X-1). The three-week horizon remains its partial mitigation |
...
| FIX-2 | signal resolution defined | T5 | `RS-1` … `RS-5` + the timestamp-ordering regression test | ✅ — **RS-5 exists**; three documents said RS-1..RS-4 until X-23 |
...
| **FIX-7** | cadence stepdown | ⛔ struck | `migrationSql > cadence_weeks rejects anything other than 1 or 2` (column only) | ⛔ struck (X-1) |
...
|---|---|---|
| IMP-1 | Availability never decayed past week 0 | `RS-5` |
...
- **`audit-report.md` is missing.** Both specs reference it for the A-series' full reasoning. The A-series rows above are traced from citations in other documents, not from the source.
