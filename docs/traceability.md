...
- **No automated check enforces this file.** X-17's proposal — a test asserting every public procedure has a limiter — is the model for what would actually keep a matrix honest. Today the only mechanism is the maintenance rule at the top, which is a habit, not a guarantee. **This gap now has an owner: T26** (`backlog.md`), raised after T25 found two procedures — `group.listMine` and `me.get` — that had been built and shipped without ever reaching `engineering-spec.md` §5. `verify:docs` could not have caught it: all seven of its checks compare documents to other documents, never to the code.
