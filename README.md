...
Every non-negotiable invariant needs a test; where one genuinely can't run yet (OV-6 needs a live Postgres with the guard trigger installed — a database-integration concern, not something a pure-function unit test can exercise), it's marked `it.todo(...)` with a comment explaining exactly what's missing, never silently skipped. See [`apps/web/tests/services/`](apps/web/tests/services/) for the full OV-1..10 / RS-1..4 suite and [`CONTRIBUTING.md`](CONTRIBUTING.md) for the invariant-needs-a-test rule.
